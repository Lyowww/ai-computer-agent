...
export function buildSystemPrompt(intent?: ClassifiedIntent): string {
  ...
  return [
    ...
    intentLock,
    "",
    "Response schema (STRICT JSON only — always include ALL four top-level keys):",
    JSON.stringify(
      {
    ...
    "",
    "Top-level fields (mandatory keys — match schema exactly):",
    '- status: one of ACTION_REQUIRED | COMPLETED | NEEDS_USER_INPUT | FAILED',
    "- reasoning_summary: short operational explanation (string; never omit the key)",
    "- actions: array of structured actions (required; never invent types/params)",
    "- message: short user-facing message (string; never omit the key)",
    "",
    "Output rules:",
    "- Return ONLY JSON. No markdown. No code fences. No explanations outside JSON.",
    "- Always include status, reasoning_summary, actions, and message — same shape as the schema.",
    ...
    "- ACTION_REQUIRED requires a non-empty actions array with valid action objects.",
    "",
